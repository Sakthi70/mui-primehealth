 ...
 */
export function hexToRgbChannel(hex: string,iscomma = false) {
  ...
  return  iscomma ? `${r}, ${g} ,${b}`:`${r} ${g} ${b}`;
}

export const createLinearGradient = (color: string,color1: string = color, opacity: number = 0.3,opacity1: number = 0.5, deg= 120): string => {
  return `linear-gradient(${deg}deg, rgba(${color}, ${opacity}), rgba(${color1}, ${opacity1}))`;
};
